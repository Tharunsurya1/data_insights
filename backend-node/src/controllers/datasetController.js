...
export const getDatasetStatus = async (req, res) => {
  ...
  try {
    ...
    const dbStatus = dataset.rows[0].status;
    ...
    if (dbStatus === 'completed') {
      ...
      try {
        ...
        // Artifacts missing - mark as failed
        return res.json({ success: true, status: "failed", error: "Pipeline completed but artifacts missing" });
      }
      ...
      try {
        ...
        await pool.query("UPDATE datasets SET status = 'completed' WHERE dataset_id = $1", [datasetId]);
        return res.json({ success: true, status: "completed" });
      } catch {
        try {
          const crashArtifactPath = path.resolve(process.cwd(), "..", "ml_engine", "data", "users", userId, datasetId, "crash.json");
          await fs.access(crashArtifactPath);
          await pool.query("UPDATE datasets SET status = 'failed' WHERE dataset_id = $1", [datasetId]);
          return res.json({ success: true, status: "failed" });
        } catch {
...
export const updateDatasetStatus = async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "UPDATE datasets SET is_active = $1 WHERE dataset_id = $2 RETURNING *",
      [status === "trained", req.params.id]
    );
    if (result.rows.length === 0) return res.status(404).json({ success: false, message: "Dataset not found" });
...
export const deleteDataset = async (req, res) => {
  ...
  try {
    ...
    const datasetCheck = await pool.query(
      "SELECT dataset_id, name FROM datasets WHERE dataset_id = $1 AND company_id = $2",
      [datasetId, companyId]
    );
